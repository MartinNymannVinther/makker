...
`git@github.com:MartinNymannVinther/makker.git`, `main` is pushed and CI
...
```bash
gh repo edit MartinNymannVinther/makker --visibility public \
  --accept-visibility-change-consequences
gh repo edit MartinNymannVinther/makker \
  --description "En at tænke højt med. Chat på EU-hostede eller lokale sprogmodeller, open source." \
  --homepage https://makker.haij.dk \
  --add-topic chat --add-topic llm --add-topic mistral --add-topic ollama \
  --add-topic nextjs --add-topic postgresql --add-topic open-source --add-topic danish
```
...
enough, and Makker can share the machine with the other Haij tools —
separate compose stack, separate database, separate domain.

Point `makker.haij.dk` at the server before you create the application in
...
BETTER_AUTH_URL       https://makker.haij.dk
LLM_PROVIDER          mistral
MISTRAL_API_KEY       the key from step 2
MAKKER_COMMIT          the short commit being deployed
```

Hex for the three database passwords: they are pasted into
`postgres://user:PASSWORD@db:5432/makker`, and a `/` from `base64` would
...
Then go straight to `https://makker.haij.dk/register` and create your
...
Set it up now rather than after the first real pile, and then do the
...
Set `DEMO=on` in Coolify and redeploy. Open `https://makker.haij.dk/demo`
in a private window and check that you land on the conversations with
the demo stripe at the top, that a conversation opens and the roles and
tasks are there, and that `/register` still shows the application form
...
an organisation's own Makker. The accounts and their workspaces expire
...
Use it for a real week. Not a demo conversation — the notes you would
otherwise have thought through alone, the draft you would otherwise have
sent unread, the minutes you would otherwise have skimmed. One week is
enough to learn what a v1 is missing; the shape of the library and the
size a conversation grows to are the likeliest first answers, and they
should be findings rather than guesses.

Until that has happened, the tool card on haij.dk waits.

## 9. The tool card

The copy for the card is written with the finish (CLAUDE.md, wave 5) as
`docs/haij-tool-card.md`, in both languages, with the proposed entry for
the site's `tools.ts`. It belongs in the haij.dk repository, not this one.
...
its job. And how often you reach for something Makker deliberately does not
