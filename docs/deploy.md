...
Makker is built to share a server with the other Haij tools: its own
...
- Point DNS for your app hostname (e.g. `makker.haij.dk`) at the server.
...
2. Set the environment variables (Coolify → Environment Variables):
   ...
   | `MAKKER_APP_PASSWORD`  | `openssl rand -hex 24`                                              |
   | `MAKKER_AUTH_PASSWORD` | `openssl rand -hex 24`                                              |
   | `BETTER_AUTH_SECRET`   | `openssl rand -base64 32`                                           |
   | `BETTER_AUTH_URL`      | `https://makker.haij.dk` (public URL; passkeys bind to this origin) |
   ...
   pasting the password into `postgres://user:PASSWORD@db:5432/makker`, and
   ...
   **Coolify pre-fills every one of these with the wrong value.** When it
   ...
   The deployed commit is not one of these. `MAKKER_COMMIT` is a build
   ...
   application never sees. Adding an environment variable to Makker means
   adding it in both places.

3. Build arguments (Coolify → Build): pass `MAKKER_COMMIT` and, if you like,
   `MAKKER_BUILT_AT`. `.dockerignore` excludes `.git`, so without them the
...
```bash
curl https://makker.haij.dk/api/health    # {"status":"ok"}
curl https://makker.haij.dk/api/version   # the release you just deployed
```
...
```bash
pg_dump -Fc -U postgres makker > makker.dump
scp makker.dump root@your-server:/root/
```

On the server:

```bash
DB=<makker db container id>   # docker ps; a shared server has more than one "db"
docker cp /root/makker.dump "$DB":/tmp/makker.dump
# The roles are provisioned by the migration step; the dump carries the data.
docker exec "$DB" pg_restore -U postgres -d makker --clean --if-exists /tmp/makker.dump
docker exec "$DB" rm /tmp/makker.dump
```
...
`https://makker.haij.dk`. Log in with your password and register a new
...
address. That is for a demo instance, not for an organisation's own Makker.
...
(Makker sends no mail yet, so you send it yourself). The link is a
...
their own workspace with the default roles and tasks in it, signed in
...
curl -X POST https://makker.haij.dk/api/demo/cleanup   # on the server
```
...
installation as an organisation's own Makker: `DEMO=on` hands out accounts,
...
Makker sends no mail. Admission links and workspace invitation links
...
repository at `scripts/backup-makker.sh`. On the VPS:

```bash
# Generate the key pair on your own machine, not on the server.
age-keygen -o makker-backup.key            # keep this in your password manager
grep 'public key' makker-backup.key        # the public half goes on the server

install -m 700 scripts/backup-makker.sh /root/backup-makker.sh
echo 'age1...' > /root/makker-backup.pub   # the public key from above
rclone config                             # configure an EU remote named eu-storage
/root/backup-makker.sh                     # run it once by hand
```

```
# crontab -e
25 2 * * * DB_CONTAINER=<makker db container id> /root/backup-makker.sh
```
...
```bash
age -d -i makker-backup.key makker-2026-01-01.dump.age | pg_restore -d makker_restore
```
