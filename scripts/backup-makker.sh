#!/usr/bin/env bash
#
# Nightly encrypted backup of the Makker database to EU object storage.
#
# Runs on the VPS, not in the container. Install as /root/backup-makker.sh
...
RECIPIENT_FILE="${RECIPIENT_FILE:-/root/makker-backup.pub}"
REMOTE="${REMOTE:-eu-storage:makker-backups}"
...
# so name Makker's explicitly (docker ps, then DB_CONTAINER=<id> in cron).
DB_CONTAINER="${DB_CONTAINER:-$(docker ps -qf name=db | head -1)}"

if [[ -z "$DB_CONTAINER" ]]; then
  echo "backup-makker: no database container found" >&2
  exit 1
fi
if [[ ! -r "$RECIPIENT_FILE" ]]; then
  echo "backup-makker: no age recipient at $RECIPIENT_FILE" >&2
  exit 1
fi

mkdir -p "$BACKUP_DIR"
STAMP=$(date +%F-%H%M)
TARGET="$BACKUP_DIR/makker-$STAMP.dump.age"
...
docker exec "$DB_CONTAINER" pg_dump -U postgres -Fc makker \
...
find "$BACKUP_DIR" -name 'makker-*.dump.age' -mtime "+$KEEP_DAYS" -delete
rclone delete --min-age "${KEEP_DAYS}d" "$REMOTE/"

echo "backup-makker: $TARGET ($(stat -c %s "$TARGET") bytes) -> $REMOTE/"
