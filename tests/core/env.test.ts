...
const REQUIRED = {
  APP_DATABASE_URL: "postgres://app:app@localhost:5432/makker",
  AUTH_DATABASE_URL: "postgres://auth:auth@localhost:5432/makker",
...
describe("the database URLs", () => {
   ...
   * postgres://user:PASSWORD@db:5432/makker. `openssl rand -base64` emits
  ...
  it("say what to do when a base64 password broke the URL", () => {
    const broken = failure({
      ...REQUIRED,
      APP_DATABASE_URL: "postgres://makker_app:ab/cd+ef=@db:5432/makker",
    });
  ...
  it("accept a hex password, which is what the guide now asks for", () => {
    expect(
      EnvSchema.safeParse({
        ...REQUIRED,
        APP_DATABASE_URL: `postgres://makker_app:${"a1b2c3".repeat(8)}@db:5432/makker`,
      }).success,
    ).toBe(true);
  });
});
...
describe("in production", () => {
  const PROD = { ...REQUIRED, NODE_ENV: "production", BETTER_AUTH_URL: "https://makker.haij.dk" };
  ...
  it("refuses a public URL that is not https", () => {
    expect(
      failure({ ...PROD, BETTER_AUTH_SECRET: STRONG, BETTER_AUTH_URL: "http://makker.haij.dk" }),
    ).toContain("https");
  });
