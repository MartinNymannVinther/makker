...
describe("rls coverage (guards future tables)", () => {
  ...
  it("runtime roles cannot bypass rls", async () => {
    const rows = await admin.query(
      `select rolname from pg_roles
       where rolname in ('makker_app', 'makker_auth') and (rolbypassrls or rolsuper)`,
    );
  ...
  /**
   * A policy granted TO PUBLIC applies to every role there is. Makker's
   * policies name makker_app or makker_auth on purpose; one written without
  ...
  const NO_AUDIT_TRIGGER = [
    ...
    "audit_log",
    // A counter for the AI ceilings: nothing anybody owns.
    "ai_calls",
    ...
    const rows = await admin.query(
      `select p.proname from pg_proc p
       where p.pronamespace = 'public'::regnamespace and p.prosecdef
         ...
         and (has_function_privilege('makker_app', p.oid, 'execute')
              or has_function_privilege('makker_auth', p.oid, 'execute'))
       order by p.proname`,
    );
  ...
  it("the ai call counter is reachable from the app role and nowhere else", async () => {
    const rows = await admin.query(
      `select has_function_privilege('makker_app', 'ai_calls_last_day()', 'execute') as app,
              has_function_privilege('makker_auth', 'ai_calls_last_day()', 'execute') as auth,
