...
describe("build stamp", () => {
  ...
  it("carries the version from package.json", () => {
    ...
    expect(env.MAKKER_VERSION).toBe(pkg.version);
  });

  it("names a commit and a migration, or says unknown", () => {
    expect(env.MAKKER_COMMIT).toMatch(/^[0-9a-f]{7,40}$|^unknown$/);
    expect(Number(env.MAKKER_MIGRATION_COUNT)).toBeGreaterThan(0);
  });
