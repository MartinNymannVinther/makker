...
beforeAll(async () => {
  ...
  // A row each, written the way the conversation service writes it:
  // through the tenant context, as the application role, as the person.
  for (const ctx of [a, b]) {
    await withOrgContext(ctx, (tx) =>
      tx
        .insert(conversations)
        .values({ orgId: ctx.orgId, userId: ctx.userId, title: `Samtale i ${ctx.orgId}` }),
    );
  }
});
...
describe("the workspace export", () => {
  it("holds one tab per table, members first, and nothing from another workspace", async () => {
    ...
    const sheet = data.sections.find((s) => s.sheet === "Samtaler")!;
    const names = sheet.rows.map((row) => row[sheet.columns.indexOf("title")]);
    expect(names).toEqual([`Samtale i ${a.orgId}`]);
  ...
  it("writes the same rows as JSON and as a workbook", async () => {
    ...
    expect(json.makker.format).toBe(1);
    expect(json.tables.Samtaler.map((row: { title: string }) => row.title)).toEqual([
      `Samtale i ${b.orgId}`,
    ]);
    ...
    expect(exportFileName("Team Æble", new Date("2026-09-11T10:00:00Z"), "xlsx")).toBe(
      "makker-team-aeble-2026-09-11.xlsx",
    );
  });
});
 ...
 * drizzle/0003_product_rls_audit.sql raises that one with ERRCODE 42501 and
