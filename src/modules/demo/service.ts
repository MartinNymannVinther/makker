 ...
 * empty, with the workspace's default roles and task library, so the
 * first conversation is a click away.
 *
 ...
 */
export function demoLandingUrl(locale: "da" | "en"): URL {
  const prefix = locale === "da" ? "" : `/${locale}`;
  return new URL(`${prefix}/chat`, env.BETTER_AUTH_URL);
}
...
export type DemoSession = { headers: Headers } | null;
