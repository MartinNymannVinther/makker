...
const version = process.env.MAKKER_VERSION || "0.0.0";
const commit = process.env.MAKKER_COMMIT || "unknown";

export const buildInfo: BuildInfo = {
  version,
  commit,
  dirty: process.env.MAKKER_DIRTY === "1",
  builtAt: process.env.MAKKER_BUILT_AT || "",
  release: formatRelease(version, commit),
  migration: process.env.MAKKER_MIGRATION || "unknown",
  migrationCount: Number(process.env.MAKKER_MIGRATION_COUNT || "0"),
};
