...
#
# - makker_app:  the application role for all domain queries, fully subject to RLS
# - makker_auth: the role Better Auth uses; only granted access to auth tables
#
...
: "${MAKKER_APP_PASSWORD:?set MAKKER_APP_PASSWORD}"
: "${MAKKER_AUTH_PASSWORD:?set MAKKER_AUTH_PASSWORD}"

psql -v ON_ERROR_STOP=1 \
  --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" \
  -v app_pw="$MAKKER_APP_PASSWORD" -v auth_pw="$MAKKER_AUTH_PASSWORD" <<'EOSQL'
SELECT format(
  CASE WHEN EXISTS (SELECT FROM pg_roles WHERE rolname = 'makker_app')
    THEN 'ALTER ROLE makker_app LOGIN PASSWORD %L'
    ELSE 'CREATE ROLE makker_app LOGIN PASSWORD %L'
  END,
  :'app_pw')
\gexec

SELECT format(
  CASE WHEN EXISTS (SELECT FROM pg_roles WHERE rolname = 'makker_auth')
    THEN 'ALTER ROLE makker_auth LOGIN PASSWORD %L'
    ELSE 'CREATE ROLE makker_auth LOGIN PASSWORD %L'
  END,
  :'auth_pw')
\gexec
EOSQL
