 ...
 * inlined into the bundle through `env`, so a running Makker can state
 ...
 * build passes MAKKER_COMMIT and MAKKER_BUILT_AT in as build arguments; git
...
const passedCommit = process.env.MAKKER_COMMIT?.trim() ?? "";
...
const builtAt = process.env.MAKKER_BUILT_AT?.trim() || new Date().toISOString();
const migration = journal.entries.at(-1)?.tag ?? "unknown";

/**
 * The content policy.
 *
 * Makker is in a good position to have a strict one: nothing in the app
 ...
 * nothing Makker serves is meant to be framed by anyone, itself included. A
...
const nextConfig: NextConfig = {
  ...
  env: {
    MAKKER_VERSION: pkg.version,
    MAKKER_COMMIT: commit,
    MAKKER_DIRTY: dirty,
    MAKKER_BUILT_AT: builtAt,
    MAKKER_MIGRATION: migration,
    MAKKER_MIGRATION_COUNT: String(journal.entries.length),
  },
  async headers() {
    return [
